    ...
    try {
      ...
      if(data[0].length == 0){
        ...
        for(let i = 0; i < arrGenre.length; i++){
          const idGenre:any = await this.sequelize.query(`select genre_id from bookmart_anime.genres where name_genre = '${arrGenre[i]}'`)
        ...
        const query = 'select genres_detail_anime_id from bookmart_anime.genres_detail where id = (select max(id) from bookmart_anime.genres_detail)'
